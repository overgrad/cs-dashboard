...
const COLOR_OVERRIDES: Record<string, string> = {
  'Cesar Lombera': 'bg-pink-500',
  'Kendall Davies': 'bg-orange-500',
  'Collins West': 'bg-yellow-500',
}
...
export function OwnerAvatar({ name }: { name: string | null }) {
  ...
  const color = COLOR_OVERRIDES[name] ?? COLORS[name.split('').reduce((a, c) => a + c.charCodeAt(0), 0) % COLORS.length]
  return (
    <span
